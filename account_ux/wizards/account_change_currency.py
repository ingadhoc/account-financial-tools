...
class AccountChangeCurrency(models.TransientModel):
    ...
    def change_currency(self):
        ...
        if self.change_type == 'value':
            if self.env['ir.module.module'].search([('name', '=', 'l10n_ar')], limit=1).state in ('installed', 'to install', 'to upgrade'):
                self.move_id.l10n_ar_currency_rate = self.currency_rate
                message = _("Currency changed from %s to %s with rate %s. Currency rate forced") % (
                    self.currency_from_id.name, self.currency_to_id.name,
                    self.currency_rate)
