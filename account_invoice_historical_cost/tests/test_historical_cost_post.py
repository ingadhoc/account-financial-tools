...
class TestHistoricalCostPost(TestStockValuationCommon):
    ...
    def test_ac10_draft_clears_and_repost(self):
        ...
        # Pending in the ORM cache, not yet in the database: without a flush
        # before the raw UPDATE, this value would be written after it and the
        # line would stay frozen in a draft invoice.
        line.historical_cost = 999.0

        invoice.button_draft()
        self.assertFalse(line.historical_cost)
        self.assertFalse(line.historical_cost_provisional)
        self.env.flush_all()
        self.env.cr.execute("SELECT historical_cost FROM account_move_line WHERE id = %s", [line.id])
