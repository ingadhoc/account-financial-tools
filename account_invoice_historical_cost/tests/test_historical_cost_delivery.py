...
class TestHistoricalCostDelivery(TestStockValuationCommon):
    ...
    def _invoice_confirmed_order(self, product, quantity):
        product.invoice_policy = "order"
        order = self.env["sale.order"].create(
            {
                "partner_id": self.partner.id,
    ...
    def test_delivery_completes_only_its_own_line(self):
        """The hook runs on every _action_done, as the inventory user, and must
        reach this order's line only — never another order's provisional one."""
        product = self.product_standard
        product.standard_price = 100.0

        pending_orders = [self._invoice_confirmed_order(product, 2) for _ in range(3)]
        order = self._invoice_confirmed_order(product, 5)

        # The core grants no read access on account.move.line to the stock
        # groups, and the hook searches them on every delivery.
        stock_user = new_test_user(
            self.env,
            login="historical_cost_stock_user",
            groups="stock.group_stock_user",
        )
        picking = order.picking_ids.with_user(stock_user)
        picking.move_ids.quantity = 5
        picking.move_ids.picked = True
        picking.button_validate()

        line = self._product_line(order.invoice_ids)
        with self.subTest("the delivered line is completed with the real cost"):
            self.assertFalse(line.historical_cost_provisional)
            self.assertAlmostEqual(line.historical_cost, 5 * 100.0)
        with self.subTest("lines of other orders stay provisional"):
            for pending_order in pending_orders:
                self.assertTrue(self._product_line(pending_order.invoice_ids).historical_cost_provisional)
