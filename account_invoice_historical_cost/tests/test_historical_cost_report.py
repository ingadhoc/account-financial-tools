...
class TestHistoricalCostReport(TestStockValuationCommon):
    ...
    def _product_lines(self, invoice):
        return invoice.line_ids.filtered(lambda line: line.display_type == "product")
    ...
    def test_real_filter_only_brings_frozen_lines(self):
        product = self.product_standard
        product.standard_price = 100.0
        real_domain = [("historical_cost_frozen", "=", True), ("historical_cost_provisional", "=", False)]

        frozen_invoice = self._create_customer_invoice(product, quantity=6)
        # As a delivery would leave it: frozen against the real cost.
        self._product_lines(frozen_invoice).historical_cost_provisional = False
        unfrozen_invoice = self._create_customer_invoice(product, quantity=6)
        # Same as a line posted before the module was installed: the column is
        # NULL, which is neither provisional nor real.
        unfrozen_invoice.line_ids._clear_historical_cost()

        product_zero = self.product_standard_auto
        product_zero.standard_price = 0.0
        zero_invoice = self._create_customer_invoice(product_zero, quantity=6)
        self._product_lines(zero_invoice).historical_cost_provisional = False

        real_moves = self.env["account.invoice.report"].search(real_domain).mapped("move_id").ids

        with self.subTest("a line frozen against the real cost is there"):
            self.assertIn(frozen_invoice.id, real_moves)
        with self.subTest("a line that was never frozen is not"):
            self.assertNotIn(unfrozen_invoice.id, real_moves)
        with self.subTest("zero is a real cost, not a missing one"):
            self.assertIn(zero_invoice.id, real_moves)
