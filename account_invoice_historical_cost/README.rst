...
#. Overrides ``account.invoice.report`` so ``price_margin`` and
   ...
   (quantity-weighted) and search filters for the lines already frozen with a
   real cost and for the ones still provisional.
...
#. With ``stock_account_multicompany_ux`` installed, a branch selling a
   product of a ``shared_to_branches`` category freezes the parent company's
   cost, the same one the core posts as COGS.
