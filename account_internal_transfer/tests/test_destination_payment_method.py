...
class TestDestinationPaymentMethod(AccountTestInvoicingCommon):
    ...
    def test_a_line_of_another_journal_is_rejected(self):
        """The view domain is not enough: a write through the ORM has to be blocked too."""
        with self.assertRaises(ValidationError):
            self._create_transfer(destination_line=self.inbound_payment_method_line)

    def test_a_line_of_the_destination_journal_of_the_wrong_type_is_rejected(self):
        """The paired payment is inbound, so an outbound line of the destination journal is not valid."""
        outbound_line = self.destination_journal.outbound_payment_method_line_ids[:1]
