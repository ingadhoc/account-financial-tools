    ...
    def _select_patch(self) -> SQL:
        return SQL(
            ...
            SELECT
                ...
                CASE
                WHEN rc.currency_id <> line.company_currency_id THEN -line.balance * currency_table.rate
                ELSE line.price_subtotal END
                AS price_subtotal,
                CASE
                WHEN rc.currency_id <> line.company_currency_id THEN currency_table.rate * -line.balance * (CASE WHEN move.move_type IN ('in_invoice','out_refund','in_receipt') THEN -1 ELSE 1 END)
                ELSE line.price_total * (CASE WHEN move.move_type IN ('in_invoice','out_refund','in_receipt') THEN -1 ELSE 1 END) END
                AS price_total,
                CASE
                WHEN  rc.currency_id <>  line.company_currency_id THEN
                 -COALESCE(
                   -- Average line price
        ...
        return SQL(
            ...
            FROM account_move_line line
                ...
                lEFT JOIN currency_rate currency_table
                    ON (
                        (currency_table.currency_id = line.currency_id
                        AND currency_table.date_start <= COALESCE(line.date, NOW())
                        AND (currency_table.date_end IS NULL OR currency_table.date_end > COALESCE(line.date, NOW()))
                        )
                        OR
