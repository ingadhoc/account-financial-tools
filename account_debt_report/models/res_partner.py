    ...
    def _get_debt_report_lines(self):
        ...
        if from_date:
            ...
            initial_line = get_line_vals(
                ...
                balance=balance,
                # the amount is stated either way: it is what the period starts from.
                # Only the running balance is left empty when it is not carried over
                amount_currency=balance_in_currency,
                balance_currency=balance_in_currency if carries_initial_currency else False,
            )
